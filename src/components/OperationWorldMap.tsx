...
export function OperationWorldMap({
  ...
  return (
    <div className="relative w-full bg-white rounded-2xl border border-zinc-200 overflow-hidden shadow-sm flex flex-col justify-between p-4 group select-none">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_120%,_rgba(59,130,246,0.02),_transparent_65%)] pointer-events-none" />

      {/* Map Header */}
      <div className="relative z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-3">
        <div className="flex items-center gap-2">
          {!hasNoExternalData && <div className="w-2h-2 rounded-full bg-emerald-500 animate-pulse" />}
          <span className="text-xs font-bold text-zinc-800 font-sans">
            원료 조달 및 제품 판매 지역 현황
          </span>
        </div>
        {!hasNoExternalData && (
          <div className="flex gap-3 text-[10px] text-zinc-600 font-semibold bg-zinc-50 px-2.5 py-1.5 rounded-lg border border-zinc-200">
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-emerald-500" /> 광양 HQ
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-indigo-600" /> 원자재 공급처 / 판매처
            </span>
          </div>
        )}
      </div>
      ...
      <div className="relative h-[300px] lg:h-[350px] w-full bg-[#f4f7fa] rounded-xl border border-zinc-200 overflow-hidden flex items-center justify-center">
        {/* Underlay Grid Gridline Pattern */}
        <div className="absolute inset-0 bg-[linear-gradient(rgba(59,130,246,0.015)_1px,_transparent_1px),_linear-gradient(90deg,_rgba(59,130,246,0.015)_1px,_transparent_1px)] bg-[size:18px_18px] opacity-80 pointer-events-none" />
        ...
        {/* Empty data overlay */}
        {hasNoExternalData ? (
          <div className="absolute inset-0 flex items-center justify-center bg-zinc-900/5 backdrop-blur-[0.5px] z-20 pointer-events-none">
            <div className="bg-white/95 border border-zinc-200 px-5 py-3 rounded-xl shadow-md text-xs font-bold text-zinc-700">
              국가별 데이터 없음
            </div>
          </div>
        ) : (
          <>
            ...
            >
              {/* Subtle Equatorial dashed divider */}
            ...
            <div className="absolute bottom-3 left-3 bg-white/95 border border-zinc-200 shadow-md rounded-lg p-2.5 flex flex-col gap-1 z-10">
              <span className="text-[8.5px] font-extrabold text-zinc-500 block font-sans">범례 (Legend)</span>
              <div className="flex items-center gap-3.5 text-[9px] font-sans text-zinc-700">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-semibold">글로벌 거점</span>
                </div>
              </div>
            </div>
          </>
