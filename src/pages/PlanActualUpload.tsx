...
export default function PlanActualUpload() {
  ...
  const selectCurrentYearToMonth = () => {
    const currentMonth = new Date().getMonth() + 1;
    setSelectedImportMonths(Array.from({ length: currentMonth }, (_, i) => i + 1));
  };

  const selectCurrentMonthOnly = () => {
    const currentMonth = new Date().getMonth() + 1;
    setSelectedImportMonths([currentMonth]);
  };
    ...
    <div className="space-y-6" onPaste={handlePaste}>
      ...
      <div className="bg-white p-6 rounded-2xl border border-[#e5e8eb] shadow-sm flex flex-col xl:flex-row justify-between items-end gap-4">
        ...
        <div className="flex flex-col gap-2 items-end shrink-0">
          ...
          {uploadTarget === '실적' && (
            <div className="w-full max-w-[340px] bg-slate-50 border border-slate-200 rounded-xl p-3 text-left space-y-2">
              ...
              <div className="flex items-center gap-1 flex-wrap text-[11px] pt-0.5 pb-0.5">
                <button
                  type="button"
                  onClick={selectAllImportMonths}
                  className="px-2 py-0.5 rounded bg-white border border-slate-200 text-brand-600 font-semibold hover:bg-brand-50 transition-colors cursor-pointer"
                >
                  전체
                </button>
                <button
                  type="button"
                  onClick={selectCurrentYearToMonth}
                  className="px-2 py-0.5 rounded bg-white border border-slate-200 text-slate-700 font-medium hover:bg-slate-100 transition-colors cursor-pointer"
                >
                  1월~현재월
                </button>
                <button
                ...
                <button
                  ...
                  className="px-2 py-0.5 rounded bg-white border border-slate-200 text-slate-500 font-medium hover:bg-slate-100 transition-colors cursor-pointer ml-auto"
                >
                  해제
                </button>
              </div>
              ...
              {selectedImportMonths.length === 0 ? (
                ...
                <div className="text-[10px] text-slate-500 leading-tight space-y-0.5 pt-1 border-t border-slate-200/60">
                  <p>• 선택한 월의 실적만 신규·중복·덮어쓰기 판정에 포함됩니다.</p>
                  <p>• 선택하지 않은 월의 기존 실적은 삭제하거나 변경하지 않습니다.</p>
                </div>
              )}
            </div>
          )}
